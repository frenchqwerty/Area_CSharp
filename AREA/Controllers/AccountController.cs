        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    {
                        var DbConnect = System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString;
                        SqlConnection sqlConnection1 = new SqlConnection(DbConnect);
                        SqlCommand cmd = new SqlCommand();
                        SqlDataReader reader;
                        object CurrentIdUser = null;

                        cmd.CommandText = "SELECT Id FROM AspNetUsers WHERE Email = '" + model.Email + "'";
                        ...
                        while (reader.Read())
                            CurrentIdUser = reader.GetSqlValue(0);
                        Environment.SetEnvironmentVariable("UserId", CurrentIdUser.ToString());
                        reader.Close();
